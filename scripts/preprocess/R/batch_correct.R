...
pca2 <- function (sce, ntop = 1000, ncomponents = 50, exprs_values = "logcounts") 
{
    assay(sce, exprs_values) <- as.matrix(assay(sce, exprs_values))
    #expr_mat <- scater:::.get_mat_for_reddim(sce, exprs_values = exprs_values, 
    #    ntop = ntop, feature_set = NULL, scale = TRUE)
    expr_mat <- scater:::.get_mat_for_reddim(logcounts(sce), subset_row = NULL, ntop = ntop, scale = TRUE)
    ncomponents <- min(c(ncomponents, dim(expr_mat)))
    pca <- prcomp(expr_mat, rank. = ncomponents)
    rownames(pca$rotation) <- colnames(expr_mat)
    return(pca)
}
...
if (args$method == "scanorama") {
  ...
  assay(sce_sel, "scanorama", withDimnames=FALSE) <- t(scanorama_mat_expr)

  pca_res <- pca2(sce_sel, ntop = 1000, ncomponents = 50, exprs_values = "scanorama")
  reducedDim(sce_sel, "PCA2") <- pca_res$x
  
  sce_sel <- runTSNE(sce_sel, dimred = "PCA2", n_dimred = 50)
  sce_sel <- runUMAP(sce_sel, dimred = "PCA2", n_dimred = 50)
